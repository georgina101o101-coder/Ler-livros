...
import { BookOpen, Check, Download, FileUp, Loader2, LogOut, Settings as SettingsIcon, Trash2 } from "lucide-react";
...
function LibraryPage() {
  ...
  const onDownload = async (id: string, name: string) => {
    ...
    try {
      const ok = await downloadForOffline(id);
      if (ok) toast.success(`"${name}" disponível offline`);
      else toast.error("Não foi possível baixar. Verifique sua conexão.");
      await refresh();
    } finally {
      setDownloading((s) => {
        ...
        {loading ? (
          ...
          <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {books.map(({ file, page }) => {
              ...
              const canOpen = file.offline || online;
              return (
                <li
                  key={file.id}
                ...
                >
                  {canOpen ? (
                    ...
                    >
                      <BookCardInner file={file} page={page} pct={pct} />
                    </Link>
                  ) : (
                    <div className="flex flex-1 flex-col gap-3 opacity-60" aria-disabled>
                      <BookCardInner file={file} page={page} pct={pct} />
                    </div>
                  )}
                  <div className="flex items-center justify-between gap-2">
                    {file.offline ? (
                      ...
                      <Button
                        ...
                        title={online ? "Baixar para leitura offline" : "Sem conexão"}
                      >
